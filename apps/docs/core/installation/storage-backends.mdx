...
Solarplan uses S3-compatible object storage for uploads in:
...
Solarplan then serves uploaded assets back through its own API.
...
This works when Solarplan and MinIO are on the same Docker network and your browser reaches MinIO through `localhost`.
...
- Solarplan: `https://cloud.solarplan.app`
- MinIO: `https://files.cloud.solarplan.app`
...
- a created bucket
- MinIO CORS allowing your Solarplan origin
...
[
  {
    ...
    "AllowedOrigins": ["https://cloud.solarplan.app"],
    "ExposeHeaders": ["ETag"]
  }
]
```
...
- a public bucket is not required for Solarplan's current private asset flow

## One copy-paste self-hosted example

This example gives you Solarplan + MinIO in one Compose file.
...
```env
SOLARPLAN_CLIENT_URL=https://cloud.solarplan.app
SOLARPLAN_API_URL=https://cloud.solarplan.app/api
...
1. Create the `solarplan-uploads` bucket.
2. Configure MinIO CORS for your Solarplan origin.
3. Make sure `files.cloud.solarplan.app` resolves publicly.
