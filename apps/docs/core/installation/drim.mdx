...
description: One-click deployment tool for self-hosted Solarplan instances
---

## What is drim?

[drim](https://github.com/usekaneo/drim) is a CLI tool for deploying your Solarplan instances. It's designed for fairly straightforward setups where you want to get Solarplan up and running quickly without manual configuration.
...
```bash
drim setup
# Enter your domain when prompted (e.g., solarplan.example.com)
...
drim provides several commands to manage your Solarplan instance:

```bash
drim setup        # Deploy Solarplan
drim start        # Start services
drim stop         # Stop services
drim restart      # Restart services
drim upgrade      # Update Solarplan to latest version
drim update       # Update drim CLI to latest version
drim configure    # Edit configuration
drim uninstall    # Remove Solarplan
```
...
- **Solarplan** - Combined API and web frontend in a single container
...
- Anyone who wants to get Solarplan running with minimal effort
