...
Solarplan supports private uploads in task descriptions and task comments.

Uploads use an S3-compatible object storage backend:

- the Solarplan API creates presigned upload URLs
- the browser uploads files directly to the storage backend
- Solarplan finalizes uploads into private asset records
- Solarplan serves uploaded files back through its own API

This means Solarplan can work with multiple backends as long as they expose a compatible S3-style API.
...
When Solarplan generates a presigned upload URL, the browser uploads directly to your storage backend.
...
If your Solarplan deployment is public, do not leave `S3_ENDPOINT` set to `http://minio:9000`.

Use a public MinIO hostname instead, for example:

```env
S3_ENDPOINT=https://files.cloud.solarplan.app
S3_BUCKET=solarplan-uploads
S3_ACCESS_KEY_ID=minioadmin
S3_SECRET_ACCESS_KEY=minioadmin
S3_REGION=us-east-1
...
- `cloud.solarplan.app` for Solarplan
- `files.cloud.solarplan.app` for MinIO

Expose MinIO on its own public hostname through your reverse proxy.

Why this is needed:

- Solarplan signs uploads against `S3_ENDPOINT`
...
Reads do not need a public bucket URL, because Solarplan serves uploaded assets back through `/api/asset/:id`.

## Other S3-compatible backends

Solarplan is not tied to MinIO. Any deployment can point Solarplan at another S3-compatible backend by changing the `S3_*` environment variables.
...
`fs` is another S3-compatible option if you want a lightweight local object store. It is built by friends of Solarplan.

When using `fs`, configure Solarplan like this:
...
- Solarplan still expects the bucket to exist before uploads start
- direct browser uploads may need reverse-proxy CORS headers, because `fs` does not currently implement S3 CORS APIs

## Required Solarplan storage variables

Set these in your Solarplan deployment:

| Name | Description |
| --- | --- |
| `S3_ENDPOINT` | S3-compatible API endpoint used by Solarplan |
...
| Name | Description |
| --- | --- |
| `S3_PUBLIC_BASE_URL` | Optional public asset base URL. Solarplan does not require this for the current private asset flow. |
