...
For the sample environment file, see the [example .env file](https://github.com/usekaneo/solarplan/blob/main/.env.sample).
...
- [Twilio (Phone OTP)](#twilio-phone-otp)
...
Name | Description |
--- | --- |
| `SOLARPLAN_CLIENT_URL` | The URL of the web application.
| `SOLARPLAN_API_URL` | The URL of the API.
...
| `DEVICE_AUTH_CLIENT_IDS` | **Optional.** Comma-separated list of allowed device authorization client IDs. When unset, Solarplan implicitly allows `solarplan-cli` and `solarplan-mcp` by default, so no extra configuration is required for the CLI or MCP. Override only when you need additional trusted clients, for example `solarplan-cli,solarplan-mcp,my-desktop-app`. |
...
Solarplan uses S3-compatible object storage for private uploads in task descriptions and task comments.
...
--- | --- | --- |
| `S3_ENDPOINT` | The S3-compatible API endpoint used by the Solarplan API for presigned uploads. Example: `http://minio:9000`. |  |
| `S3_BUCKET` | The bucket Solarplan will use for uploaded files. |  |
| `S3_ACCESS_KEY_ID` | Access key used by the Solarplan API to create presigned upload URLs. |  |
| `S3_SECRET_ACCESS_KEY` | Secret key used by the Solarplan API to create presigned upload URLs. |  |
| `S3_REGION` | The storage region used for request signing. | `us-east-1` |
| `S3_PUBLIC_BASE_URL` | Optional public base URL for uploaded assets. Solarplan does not require this for the current private asset flow. |  |
...
| `S3_KEY_PREFIX` | Optional prefix prepended to all S3 object keys. Useful for sharing a single bucket across multiple environments (e.g. `staging/`, `prod/solarplan/`). Trailing slashes are trimmed automatically. |  |
...
Solarplan supports Redis for horizontal scaling of WebSocket broadcasts. When any Redis mode is configured, multiple API instances can relay real-time updates via Redis Pub/Sub. When none are set, an in-memory adapter is used (single-instance only).
...
Phone number sign-in sends a one-time code via SMS using [Twilio](https://www.twilio.com/docs/verify). Create a Twilio account, buy/verify a sending phone number, and set the following variables. If unset, phone sign-in falls back to logging the OTP to the server console in non-production environments only — no SMS is sent.

Name | Description |
--- | --- |
| `TWILIO_ACCOUNT_SID` | Your Twilio Account SID.
| `TWILIO_AUTH_TOKEN` | Your Twilio Auth Token.
| `TWILIO_PHONE_NUMBER` | The Twilio phone number OTP messages are sent from, in E.164 format (e.g. `+15551234567`).
...
- If you enable Twilio, phone number sign-in will send OTP codes via SMS. See [Twilio (Phone OTP)](#twilio-phone-otp).
- If you need device authorization for clients beyond the defaults (`solarplan-cli`, `solarplan-mcp` when `DEVICE_AUTH_CLIENT_IDS` is unset), set `DEVICE_AUTH_CLIENT_IDS` to the full comma-separated list of trusted client IDs for your deployment.
