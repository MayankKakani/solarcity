...
description: Configure Solarplan with your GitHub App credentials and connect projects to repositories. Includes Docker Compose and Kubernetes examples.
---


This guide covers configuring Solarplan with your GitHub App credentials and connecting projects to repositories.
...
Add the following environment variables to your Solarplan deployment:
...
GITHUB_APP_NAME=solarplan-your-instance-name
```
...
| `GITHUB_APP_NAME` | App slug for installation URLs in the UI | ⚠️ | `solarplan-mycompany` |
  ...
  backend:
    ...
    environment:
      ...
      GITHUB_PRIVATE_KEY: |
        ...
        -----END RSA PRIVATE KEY-----
      GITHUB_WEBHOOK_SECRET: "your-webhook-secret-here"
      GITHUB_APP_NAME: "solarplan-mycompany"
...
1. Solarplan will verify that your GitHub App has access
...
Create a new task in your connected Solarplan project with:
- A clear title
- A description
...
- A comment links back to the Solarplan task
</Step>
...
1. Change task priority in Solarplan → Check issue labels in GitHub
2. Add/remove labels in GitHub → Check task in Solarplan
3. Change task status in Solarplan → Check if issue is closed/reopened
</Step>
...
Configure how Solarplan matches branches to tasks in your project's GitHub integration settings:
...
Solarplan automatically creates and syncs these label types:
...
**Custom Labels:**
- Any labels added to GitHub issues (not starting with `priority:` or `status:`) are synced to Solarplan
- Labels added in Solarplan can be synced to GitHub (if configured)
...
3. Each Solarplan project can connect to any repository where your app is installed
