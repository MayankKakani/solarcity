...
description: Send task activity from a Solarplan project to Slack, Discord, Telegram, or any HTTP endpoint. Configure per-project notifications and optional signing.
---

Solarplan can notify external systems when tasks change in a project. Each integration is per project and uses either an incoming webhook URL from the service you connect (Slack, Discord), a bot token plus chat ID (Telegram), or your own HTTP listener (generic webhook).

<Info>
These integrations are outgoing only: Solarplan posts to your URL when events occur. They are separate from [GitHub integration](/core/integrations/github/setup), which syncs issues and repositories.
</Info>
...
Notification messages and generic webhook payloads include links to the task in the web app. Set [`SOLARPLAN_CLIENT_URL`](/core/installation/environment-variables) to your public Solarplan web URL so those links point to your instance (for example `https://solarplan.example.com`).
...
Solarplan sends formatted messages to Slack when enabled events occur.
...
After delivery attempts, Solarplan stores basic health metadata (for example last success, last failure message, failure count) so you can see whether recent posts succeeded. This does not replace monitoring on your endpoint.
...
- `timestamp`: ISO 8601 time when Solarplan sent the request
...
If you configure a secret for the generic webhook integration, Solarplan signs each request and sends the result in the `X-Solarplan-Signature` header.

- Header: `X-Solarplan-Signature`
- Algorithm: `HMAC-SHA256`
- Signed input: the exact raw request body Solarplan sends
...
3. Compare your computed lowercase hex digest with `X-Solarplan-Signature` using a constant-time comparison.
4. Reject the request if the signatures do not match.

Solarplan currently signs only the raw request body. It does not include a separate timestamp value in the signature input or send a dedicated timestamp header for replay protection. If you add replay defenses in front of your receiver, document that behavior separately so it does not drift from Solarplan's payload contract.
