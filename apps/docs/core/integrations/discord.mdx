---
title: Discord
description: Send task notifications from Solarplan to a Discord channel using incoming webhooks.
---

Solarplan can post task activity to a Discord channel whenever events occur in a project. This uses Discord's incoming webhook feature — no bot or OAuth setup required.
...
4. Click **New Webhook**.
5. Give the webhook a name (e.g. "Solarplan") and optionally set an avatar.
...
Solarplan sends Discord embed messages with:

- A title describing the event (e.g. "Task Status Changed")
- Task name with a clickable link to the task in Solarplan
