...
description: Configure Solarplan with your custom OpenID Connect (OIDC) provider.
---

## Overview

Solarplan supports custom OAuth 2.0 and OpenID Connect (OIDC) providers, allowing you to integrate with any standards-compliant identity provider such as Keycloak, Auth0, Okta, Azure AD, or self-hosted solutions like Pocket ID.
...
3. Set the redirect URI to: `{SOLARPLAN_API_URL}/api/auth/oauth2/callback/custom`
   - Example: `https://api.solarplan.example.com/api/auth/oauth2/callback/custom`
...
After updating the environment variables, restart your Solarplan services:
...
```bash
CUSTOM_OAUTH_CLIENT_ID=solarplan-client
...
```
{SOLARPLAN_API_URL}/api/auth/oauth2/callback/custom
```

For example, if `SOLARPLAN_API_URL=https://api.solarplan.example.com`, the redirect URI should be:

```
https://api.solarplan.example.com/api/auth/oauth2/callback/custom
```
