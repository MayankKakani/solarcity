...
All API endpoints require authentication. Solarplan currently supports two authentication patterns for API access:
...
Use API keys when you control secret storage and want a stable credential for a single Solarplan instance.
...
The device flow follows RFC 8628 and returns a Bearer token that can be used against that Solarplan instance's API.

## Creating an API Key

<Steps>
<Step>
**Sign in to Solarplan**

Sign in to your Solarplan instance using your account credentials.
</Step>
...
```bash
curl -X GET https://your-solarplan-instance.com/api/task?workspaceId=your-workspace-id \
...
```javascript
const response = await fetch('https://your-solarplan-instance.com/api/task?workspaceId=your-workspace-id', {
  method: 'GET',
  headers: {
    'Authorization': 'Bearer your-api-key-here',
...
By default, self-hosted Solarplan allows the built-in device clients `solarplan-cli` and `solarplan-mcp`.
...
2. Solarplan returns a `device_code`, `user_code`, polling interval, and verification URL
...
```bash
curl -X POST https://your-solarplan-instance.com/api/auth/device/code \
  -H "Content-Type: application/json" \
...
```json
{
  "device_code": "dev_123",
  "user_code": "ABCD-1234",
  "verification_uri": "https://your-solarplan-instance.com/device",
  "verification_uri_complete": "https://your-solarplan-instance.com/device?user_code=ABCD-1234",
  "interval": 5,
  "expires_in": 600
}
```
...
```bash
curl -X POST https://your-solarplan-instance.com/api/auth/device/token \
  -H "Content-Type: application/json" \
  -d '{
...
While waiting for approval, Solarplan may return:
...
Once approved, Solarplan returns an access token:
