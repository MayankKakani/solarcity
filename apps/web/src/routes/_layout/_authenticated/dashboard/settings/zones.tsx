...
function RouteComponent() {
  ...
  const selectedZoneMatch = location.pathname.match(
    /^\/dashboard\/settings\/zones\/([^/]+)\//,
  );
  const selectedzoneId = selectedZoneMatch?.[1] || "";
  ...
  useEffect(() => {
    const isProjectsRoot =
      location.pathname === "/dashboard/settings/zones" ||
      location.pathname === "/dashboard/settings/zones/";
    ...
    void navigate({
      ...
      params: { zoneId: projects[0].id },
      replace: true,
    });
  }, [location.pathname, navigate, projects]);

  const selectedProject = projects?.find(
    (project) => project.id === selectedzoneId,
  );
            ...
            <SidebarGroupContent>
              <Select
                ...
                onValueChange={(zoneId) => {
                  ...
                  void navigate({
                    to: `/dashboard/settings/zones/${zoneId}/${nextSegment}`,
                  });
                }}
              >
                <SelectTrigger
              ...
              <SidebarMenu className="gap-0.5">
                {menuItems.map((item) => {
                  const toUrl = selectedProject
                    ? `/dashboard/settings/zones/${selectedProject.id}/${item.segment}`
                    : "/dashboard/settings/zones";
