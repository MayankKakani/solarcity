...
function KaneoIssueLinkView({ node }: NodeViewProps) {
  ...
  const { data: task } = useQuery({
    queryKey: ["task", taskId, "solarplan-issue-link"],
  ...
  const { data: project } = useQuery({
    ...
    enabled: Boolean(taskRoute?.workspaceId && taskRoute?.zoneId),
    staleTime: 1000 * 60,
  });
  ...
  const href =
    taskRoute?.workspaceId && taskRoute?.zoneId && task?.id
      ? `/dashboard/workspace/${taskRoute.workspaceId}/project/${taskRoute.zoneId}/task/${task.id}`
      : url;
  const isInternal = href.startsWith("/");

  return (
    <NodeViewWrapper as="span" className="solarplan-issue-link-node">
      <HoverCard openDelay={160} closeDelay={120}>
        <HoverCardTrigger asChild>
          <a
            ...
            className="solarplan-issue-link-chip"
          >
            {resolvedIssueKey ? (
              <span className="solarplan-issue-link-key">
                {resolvedIssueKey}
              </span>
            ) : null}
            <span className="solarplan-issue-link-title">{title}</span>
          </a>
        </HoverCardTrigger>
        <HoverCardContent
          side="top"
          align="start"
          sideOffset={8}
          className="solarplan-issue-link-preview"
        >
          <div className="solarplan-issue-link-preview-top">
            <span className="solarplan-issue-link-preview-key">
              {resolvedIssueKey || t("tasks:entity.task")}
            </span>
            <span className="solarplan-issue-link-preview-assignee">
              {assignee}
            </span>
          </div>
          <p className="solarplan-issue-link-preview-title">{title}</p>
          <div className="solarplan-issue-link-preview-meta">
            <span>{status}</span>
            <span>·</span>
            <span>{priority}</span>
          </div>
...
export const KaneoIssueLink = Node.create({
  ...
  parseHTML() {
    return [
      { tag: "solarplan-issue-link[url]" },
      { tag: "span[data-type='solarplan-issue-link'][data-url]" },
    ];
  },
  ...
  renderHTML({ HTMLAttributes }) {
    return [
      ...
      mergeAttributes(HTMLAttributes, {
        "data-type": "solarplan-issue-link",
  ...
  renderMarkdown(
    ...
    if (!url) return "";
    return `\n<solarplan-issue-link url="${url}" issue-key="${issueKey}" task-id="${taskId}" />\n`;
  },
});
