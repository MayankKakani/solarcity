...
# Process SOLARPLAN_API_URL first (with special handling)
if [ ! -z "$SOLARPLAN_API_URL" ]; then
  echo "Found SOLARPLAN_API_URL: $SOLARPLAN_API_URL"

  # First, replace the exact string "SOLARPLAN_API_URL" in all JavaScript files
  # Use grep -l to only process files that contain the string
  find /usr/share/nginx/html -type f -name "*.js" -exec grep -l "SOLARPLAN_API_URL" {} \; | xargs -r sed -i "s#SOLARPLAN_API_URL#$SOLARPLAN_API_URL#g"
  ...
  BASE_URL=$(echo "$SOLARPLAN_API_URL" | sed 's#/api/*$##')
  ...
  echo "✅ Replaced SOLARPLAN_API_URL with $SOLARPLAN_API_URL"
else
  echo "WARNING: SOLARPLAN_API_URL environment variable is not set. API calls may fail."
...
# Process SOLARPLAN_CLIENT_URL efficiently
if [ ! -z "$SOLARPLAN_CLIENT_URL" ]; then
  echo "Found SOLARPLAN_CLIENT_URL: $SOLARPLAN_CLIENT_URL"
  
  # Only process files that actually contain the string
  find /usr/share/nginx/html -type f -name "*.js" -exec grep -l "SOLARPLAN_CLIENT_URL" {} \; | xargs -r sed -i "s#SOLARPLAN_CLIENT_URL#$SOLARPLAN_CLIENT_URL#g"
  find /usr/share/nginx/html -type f -name "*.js" -exec grep -l "\"SOLARPLAN_CLIENT_URL\"" {} \; | xargs -r sed -i "s#\"SOLARPLAN_CLIENT_URL\"#\"$SOLARPLAN_CLIENT_URL\"#g"
  
  echo "✅ Replaced SOLARPLAN_CLIENT_URL with $SOLARPLAN_CLIENT_URL"
fi

# Process any other KANEO_ prefixed environment variables (for future extensibility)
# Exclude the ones we've already processed
for key in $(env | grep '^KANEO_' | grep -v 'SOLARPLAN_API_URL\|SOLARPLAN_CLIENT_URL' | cut -d= -f1); do
  value=$(printenv "$key")
