...
# Derive SOLARPLAN_API_URL from SOLARPLAN_CLIENT_URL if not explicitly set
client_url_trimmed="$(printf '%s' "${SOLARPLAN_CLIENT_URL:-}" | sed 's/^[[:space:]]*//;s/[[:space:]]*$//')"
client_url="${client_url_trimmed%/}"
if [ -z "${SOLARPLAN_API_URL:-}" ] && [ -n "$client_url" ]; then
  export SOLARPLAN_API_URL="${client_url}/api"
  echo "SOLARPLAN_API_URL not set — derived from SOLARPLAN_CLIENT_URL: $SOLARPLAN_API_URL"
fi
...
if [ -z "${DATABASE_URL:-}" ]; then
  POSTGRES_DB="${POSTGRES_DB:-solarplan}"
  POSTGRES_USER="${POSTGRES_USER:-solarplan}"
