...
describe("github labels helpers", () => {
  ...
  it("creates missing labels only when needed", async () => {
    ...
    await ensureLabelsExist(octokit as never, "usekaneo", "solarplan", [
      "status:done",
      "priority:high",
    ]);
  ...
  it("adds labels after ensuring they exist", async () => {
    ...
    await addLabelsToIssue(octokit as never, "usekaneo", "solarplan", 12, [
      "priority:low",
      "status:done",
    ]);
  ...
  it("ignores missing labels during removal", async () => {
    ...
    await expect(
      removeLabel(octokit as never, "usekaneo", "solarplan", 21, "status:done"),
