...
describe("prepare-database-startup", () => {
  ...
  it("surfaces safe database details and skips migrations when readiness fails", async () => {
    ...
    await expect(
      ...
      "Database startup failed for postgres:5432/solarplan (source: POSTGRES_ENV). If you are running outside Docker Compose, use localhost or set DATABASE_URL explicitly.",
    );
  ...
  it("logs migration failures separately from readiness failures", async () => {
    ...
    await expect(
      ...
      "Database migrations failed for postgres:5432/solarplan (source: POSTGRES_ENV).",
    );
